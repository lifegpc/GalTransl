...
DEEPSEEK_BGI_TRANS_PROMPT = """# 输入说明
代码块中包含了一段[SourceLang]视觉小说脚本，以jsonline格式呈现。
...
1. `<rX>` 标签：
  - 严格保留原始标签的存在
  - 绝不添加源文本中不存在的新 `<rX>` 标签
  - 仅修改 X 内容，同时保持标签结构不变（如果存在）
    示例：
    a) 输入：`<rAshita>明日</r>` → 合法输出：`<rTomorrow>明天</r>`
    b) 输入：`明日` → 禁止的输出：`<rTomorrow>明天</r>`
2. 必须保持所有注释的定位绑定完整性
...
DEEPSEEK_KRKR_TRANS_PROMPT = """# 输入说明
代码块中包含了一段[SourceLang]视觉小说脚本，以jsonline格式呈现。
...
# 特殊格式规则
1. `[X]` 注释：
  - 保持注释的存在性：源文本中无注释 → 输出文本中无注释
  - 在源文本包含注释的情况下，允许动态调整注释数量
  - 每个注释必须仍然与其后字符绑定
   例子：
   a) 输入："普通文本" → 输出："普通文本"（不添加注释）
   b) 输入：`[Ashi]明[ta]日` → 输出：`[míng]明[tiān]天`（1:1 注释）
   c) 输入：`[Do]土[you]曜[bi]日` → 输出：`[zhōu]周[liù]六`（减少注释）
   d) 输入：`土曜日` → 禁止输出：`[zhōu]周[liù]六`（添加额外注释）  
2. 位置绑定完整性必须对所有注释保持一致
